...
reformat.ASTRAL.tree.for.Julia <- function(tree_path, add.arbitrary.terminal.branches = FALSE){
  # Open tree
...
# ASTRAL does not output terminal branch lengths, so these are arbitrarily given the length 1
# ASTRAL includes posterior probabilities: these are stripped (the Julia function cannot handle extra node values)
reformat.gene.tree.list.for.Julia <- function(trees_path, add.arbitrary.terminal.branches = FALSE){
  # Open trees
  ts <- read.tree(trees_path)
  for (i in 1:length(ts)){
    temp_tree <- ts[[i]]
    temp_tree <- reformat.phylo.for.Julia(temp_tree, add.arbitrary.terminal.branches)
    ts[[i]] <- temp_tree
  }
...
reformat.phylo.for.Julia <- function(tree, add.arbitrary.terminal.branches = FALSE){
